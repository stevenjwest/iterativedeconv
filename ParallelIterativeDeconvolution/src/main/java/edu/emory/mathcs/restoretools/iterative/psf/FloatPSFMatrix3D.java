...
public class FloatPSFMatrix3D {
    ...
    /**
     * Creates a new instance of FloatPSFMatrix3D.
     * 
     * @param imPSF
     *            array of PSF images
     * @param boundary
     ...
     */
    public FloatPSFMatrix3D(ImagePlus[][][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
        PSF = new FloatPSF3D(imPSF);
    ...
    public FloatMatrix3D times(FloatMatrix3D B, boolean transpose) {
        ...
        case INVARIANT:
            switch (boundary) {
            case ZERO:
                Bpad = FloatCommon3D.padZero(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
                break;
            case PERIODIC:
                Bpad = FloatCommon3D.padPeriodic(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
                break;
            case REFLEXIVE:
                Bpad = FloatCommon3D.padReflexive(B, invPsfSize[0], invPsfSize[1], invPsfSize[2]);
                break;
            }
        ...
        case VARIANT:
            switch (boundary) {
            case ZERO:
                Bpad = FloatCommon3D.padZero(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
                break;
            case REFLEXIVE:
                Bpad = FloatCommon3D.padReflexive(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
                break;
            case PERIODIC:
                Bpad = FloatCommon3D.padPeriodic(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1], imSize[2] + 2 * varPadSize[2]);
                break;
            }
            if (transpose) {
    ...
    private FloatMatrix3D invariantMultiply(AbstractMatrix3D Bpad, boolean transpose) {
        ...
        return ((FComplexMatrix3D) Bpad).viewPart(invPadSize[0], invPadSize[1], invPadSize[2], imSize[0], imSize[1], imSize[2]).getRealPart();
    }

    private FloatMatrix3D variantMultiply(FloatMatrix3D Bpad) {
        if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
            Bpad = FloatCommon3D.padZero(Bpad, varPadSize1, PaddingType.POST);
        }
        ...
        for (int i = 0; i < varNregions[0]; i++) {
            for (int j = 0; j < varNregions[1]; j++) {
                for (int k = 0; k < varNregions[2]; k++) {
                    ...
                    Xt = FloatCommon3D.padZero(Xt, varPadSize, PaddingType.BOTH);
    ...
    private FloatMatrix3D variantMultiplyOnePsf(int slice, int row, int col, FloatMatrix3D X, boolean transpose) {
        ...
        if ((padSize1[0] > 0) || padSize1[1] > 0 || padSize1[2] > 0) {
            X = FloatCommon3D.padZero(X, padSize1, PaddingType.POST);
        }
    ...
    private FloatMatrix3D multiplyOneRegion(int slice, int row, int col, AbstractMatrix3D X, boolean transpose) {
        ...
        if ((padSize[0] > 0) || padSize[1] > 0 || padSize[2] > 0) {
            X = FloatCommon3D.padZero((FloatMatrix3D) X, padSize, PaddingType.POST);
        } else {
            if (X.isView()) {
