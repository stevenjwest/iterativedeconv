...
public class FloatPSFMatrix2D {
    ...
    /**
     * Creates a new instance of FloatPSFMatrix2D.
     * 
     * @param imPSF
     *            array of PSF images
     * @param boundary
     ...
     */
    public FloatPSFMatrix2D(ImagePlus[][] imPSF, BoundaryType boundary, ResizingType resizing, int[] imSize) {
        PSF = new FloatPSF2D(imPSF);
    ...
    public FloatMatrix2D times(FloatMatrix2D B, boolean transpose) {
        ...
        case INVARIANT:
            switch (boundary) {
            case ZERO:
                Bpad = FloatCommon2D.padZero(B, invPsfSize[0], invPsfSize[1]);
                break;
            case PERIODIC:
                Bpad = FloatCommon2D.padPeriodic(B, invPsfSize[0], invPsfSize[1]);
                break;
            case REFLEXIVE:
                Bpad = FloatCommon2D.padReflexive(B, invPsfSize[0], invPsfSize[1]);
                break;
            }
        ...
        case VARIANT:
            switch (boundary) {
            case ZERO:
                Bpad = FloatCommon2D.padZero(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
                break;
            case REFLEXIVE:
                Bpad = FloatCommon2D.padReflexive(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
                break;
            case PERIODIC:
                Bpad = FloatCommon2D.padPeriodic(B, imSize[0] + 2 * varPadSize[0], imSize[1] + 2 * varPadSize[1]);
                break;
            }
            if (transpose) {
    ...
    private FloatMatrix2D invariantMultiply(AbstractMatrix2D Bpad, boolean transpose) {
        Bpad = ((DenseFloatMatrix2D) Bpad).getFft2();
        if (transpose) {
        ...
        ((DenseFComplexMatrix2D) Bpad).ifft2(true);
        return ((FComplexMatrix2D) Bpad).viewPart(invPadSize[0], invPadSize[1], imSize[0], imSize[1]).getRealPart();
    }

    private FloatMatrix2D variantMultiply(FloatMatrix2D Bpad) {
        if ((varPadSize1[0] > 0) || (varPadSize1[1] > 0)) {
            Bpad = FloatCommon2D.padZero(Bpad, varPadSize1, PaddingType.POST);
        }
    ...
    private FloatMatrix2D variantTransposeMultiply(FloatMatrix2D Bpad) {
        ...
        for (int i = 0; i < varNregions[0]; i++) {
            for (int j = 0; j < varNregions[1]; j++) {
                ...
                Xt = FloatCommon2D.padZero(Xt, varPadSize, PaddingType.BOTH);
    ...
    private FloatMatrix2D variantMultiplyOnePsf(int row, int col, FloatMatrix2D X, boolean transpose) {
        ...
        if ((padSize1[0] > 0) || padSize1[1] > 0) {
            X = FloatCommon2D.padZero(X, padSize1, PaddingType.POST);
        }
    ...
    private FloatMatrix2D multiplyOneRegion(int row, int col, AbstractMatrix2D X, boolean transpose) {
        ...
        if ((padSize[0] > 0) || padSize[1] > 0) {
            X = FloatCommon2D.padZero((FloatMatrix2D) X, padSize, PaddingType.POST);
        } else {
            if (X.isView()) {
                X = ((FloatMatrix2D) X).copy();
            }
        }
        X = ((DenseFloatMatrix2D) X).getFft2();
        if (transpose) {
        ...
        ((DenseFComplexMatrix2D) X).ifft2(true);
